extends Node2D

const GRID_SIZE = Vector2(20, 20)

enum MoveFlags {
	MOVE_PUSH = 1 << 0
	MOVE_ENTER = 1 << 1
}
const MOVE_DEFAULT = MOVE_PUSH | MOVE_ENTER

var _moving = false
var movement_time = 0.2 # s
onready var tween = $tween
export var mass = 1

func _ready():
	get_parent().update_entity_position(self, get_grid_position())

func get_grid_position():
	return (position / GRID_SIZE).floor()

func set_grid_position(grid_position):
	position = grid_position.floor() * GRID_SIZE
	
	get_parent().update_entity_position(self, get_grid_position())

func move(offset, strength = mass, flags = MOVE_DEFAULT): # -> bool (could move)
	strength -= mass
	if offset.length_squared() < 0.01:
		return true
	
	if _moving or strength < 0:
		return false
	
	var original_moving_position = $moving.global_position
	
	var to_push = get_parent().get_entity_at_position(get_grid_position() + offset)
	if to_push != null:
		if not (bool(flags & MOVE_PUSH) and to_push.move(offset, strength, flags)):
			return false # Likely not enough strength
	
	_moving = true
	
	position += offset * GRID_SIZE
	
	get_parent().update_entity_position(self, get_grid_position())
	
	tween.remove_all()
	$moving.global_position = original_moving_position 
	tween.interpolate_property(
		$moving, "global_position",
		original_moving_position, global_position,
		movement_time, Tween.TRANS_CUBIC, Tween.EASE_OUT)
	tween.start()
	yield(tween, "tween_completed")
	
	_moving = false
